...
| 우선순위 | 기능 | 설명 |
|----------|------|------|
| Phase 2 | 벳 사이징 실전 모드 | 팟 사이즈/상황별로 실제 배팅 금액을 입력하며 연습. 현재는 액션(레이즈/폴드) 선택만 → 금액까지 포함 |
| Phase 2 | Bot 모의 게임 | AI 봇과 1:1 핸드를 플레이하며 프리플랍~리버까지 의사결정 연습. 실전 감각 훈련 |
| Phase 2 | 스페이스드 레피티션 | 완료 레슨 시간 경과 시 "복습 필요" 표시. 장기 기억 강화 |
| Phase 2 | 시간 기반 하트 회복 | 틀리면 하트 소모, 시간 경과로 회복. 게임성 강화 |
